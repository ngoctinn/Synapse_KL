...
*   **Form Pattern**: Dùng `form.tsx` của shadcn (`Form`, `FormField`, `FormItem`, `FormControl`, `FormMessage`) + `zodResolver`.
*   **Strict Typing**: Zod Schema Inference (`z.infer<typeof schema>`) + generic cho `useForm<T>()`.
